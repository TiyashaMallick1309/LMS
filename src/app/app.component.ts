...
export class AppComponent {
  ...
  cartItems: { item: Book; qty: number }[] = [];
  total = 0;
  ...
  get filteredBooks(): Book[] {
    ...
    return this.books.filter(book => book.title.toLowerCase().includes(searchTerm));
  }
  ...
  addToCart(book: Book) {
    const itemId = book.ISBN;
    const itemIndex = this.cartItems.findIndex(x => x.item.ISBN === itemId);
    if (itemIndex === -1) {
      this.cartItems.push({ item: book, qty: 1 });
    } else {
      this.cartItems[itemIndex].qty++;
    }
    this.total += book.price.value; 
  }

  removeFromCart(index: number) {
    const item = this.cartItems[index];
    this.cartItems.splice(index, 1);
    this.total -= item.item.price.value * item.qty;
  }

  getCartItemQty(itemId: number): number {
    const item = this.cartItems.find(x => x.item.ISBN === itemId);
  ...
  getTotalPrice(): number {
    ...
    this.cartItems.forEach(item => {
      totalPrice += item.item.price.value * item.qty;
    });
    return totalPrice;
  }

  updateQuantity(cartItem: any, operation: string) {
    if (operation === 'increment') {
      ...
      if (cartItem.qty > 1) {
        ...
        this.removeFromCart(this.cartItems.indexOf(cartItem));
      }
    }
  }
